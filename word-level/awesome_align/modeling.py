...
class BertGuideHead(nn.Module):
    ...
    def forward(
        ...
        guide=None,
        extraction='softmax', softmax_threshold=0.001, softmax_omit_threshold=None,
        ...
        if guide is None:
            ...
            threshold_omit = softmax_omit_threshold if extraction == 'softmax' else 0

            if threshold_omit:
                align_matrix = torch.full(attention_probs_src.shape, 0, dtype=torch.long)
                neg_pos_flag = (attention_probs_src>threshold_omit)*(attention_probs_tgt>threshold_omit)
                align_matrix[neg_pos_flag] = 2 
                align_flag = (attention_probs_src>threshold)*(attention_probs_tgt>threshold)
                align_matrix[align_flag] = 1 
...
class BertContrastiveHead(nn.Module):
    ...
    def forward(
        ...
        inputs_src, inputs_tgt,
        guide=None,
        extraction='softmax', softmax_threshold=0.001, softmax_omit_threshold=0,
        output_prob=False,
        temp=1
    ):
        ...
        # normalization
        hidden_states_src = hidden_states_src.div(torch.norm(hidden_states_src, dim=-1).unsqueeze(-1))
        hidden_states_tgt = hidden_states_tgt.div(torch.norm(hidden_states_tgt, dim=-1).unsqueeze(-1))

        attention_mask_src_bool = attention_mask_src==0
        attention_mask_tgt_bool = attention_mask_tgt==0
        ...
        # 1. softmax version 
        # attention_scores_src = attention_scores + attention_mask_tgt # mask 的位置赋一个 inf，在 softmax 置为 0
        # attention_scores_tgt = attention_scores + attention_mask_src.transpose(-1, -2)
        ...
        # 2. mask exp version
        attention_scores_src = attention_scores * attention_mask_tgt_bool
        attention_scores_tgt = (attention_scores.transpose(2, 3) * attention_mask_src_bool).transpose(2, 3) 

        attention_probs_src = attention_scores_src
        attention_probs_tgt = attention_scores_tgt
        ...
        wc_loss_src, wc_loss_tgt, wc_loss = 0, 0, 0
        align_flag = guide.bool()
        if softmax_omit_threshold:
            align_flag = guide == 1.0
            neg_pos_flag = guide != 0.0
        for i in range(attention_probs_src.size()[0]):
            src_length = torch.sum(attention_mask_src[i][0][0]==0, dim=-1).tolist()
            seq_loss = 0
            for j in range(1, src_length+1):
                top = torch.sum( torch.exp(attention_probs_src[i][0][j][align_flag[i][0][j]] / temp), dim=-1)
                if softmax_omit_threshold:
                    bottom = torch.sum( torch.exp(attention_probs_src[i][0][j][neg_pos_flag[i][0][j]] / temp), dim=-1) + 1e-9
                else:
                    bottom = torch.sum( torch.exp(attention_probs_src[i][0][j])*(attention_mask_tgt[i][0]==-0), dim=-1) + 1e-9
                seq_loss = seq_loss + top / bottom
            mean_seq_loss = seq_loss / src_length
            wc_loss_src = wc_loss_src + mean_seq_loss
        wc_loss_src = wc_loss_src / attention_probs_src.size()[0]

        for i in range(attention_probs_tgt.size()[0]):
            tgt_length = torch.sum(attention_mask_tgt[i][0][0]==0, dim=-1).tolist()
            seq_loss = 0
            for j in range(1, tgt_length+1):
                top = torch.sum( torch.exp(attention_probs_tgt.transpose(2,3)[i][0][j][align_flag.transpose(2,3)[i][0][j]] / temp) ,dim=-1)
                if softmax_omit_threshold:
                    bottom = torch.sum( torch.exp(attention_probs_tgt.transpose(2,3)[i][0][j][neg_pos_flag.transpose(2,3)[i][0][j]] / temp), dim=-1 ) + 1e-9
                else:
                    bottom = torch.sum( torch.exp(attention_probs_tgt.transpose(2,3)[i][0][j])*(attention_mask_src[i][0]==0), dim=-1 ) + 1e-9
                seq_loss = seq_loss + top / bottom
            mean_seq_loss = seq_loss / tgt_length
            wc_loss_tgt = wc_loss_tgt + mean_seq_loss
        wc_loss_tgt = wc_loss_tgt / attention_probs_tgt.size()[0]

        wc_loss = wc_loss_src + wc_loss_tgt
        wc_loss = -wc_loss
        ...
        #     so_loss_src = torch.sum(torch.sum (attention_probs_src*guide, -1), -1).view(-1)
        #     so_loss_tgt = torch.sum(torch.sum (attention_probs_tgt*guide, -1), -1).view(-1)
        # attention_probs_src: [bsz, 1, src_len, tgt_len]
        # torch.sum (attention_probs_src*guide, -1): [bsz, 1, src_len]
        # so_loss_src: [bsz]

        #     so_loss = so_loss_src/len_src + so_loss_tgt/len_tgt # 根据句子长度也就是 token 个数取平均
        #     so_loss = -torch.mean(so_loss) # 根据 batch 大小 也就是 句子个数 取平均
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def get_aligned_word(self, inputs_src, inputs_tgt, bpe2word_map_src, bpe2word_map_tgt, device, src_len, tgt_len, align_layer=8, extraction='softmax', softmax_threshold=0.001, softmax_omit_threshold=None, test=False, output_prob=False, word_aligns=None):
        batch_size = inputs_src.size(0)
        ...
        if word_aligns is None:
            ...
            with torch.no_grad():
                ...
                attention_probs_inter = self.guide_layer(outputs_src, outputs_tgt, inputs_src, inputs_tgt, extraction=extraction, softmax_threshold=softmax_threshold, softmax_omit_threshold=softmax_omit_threshold, output_prob=output_prob)
                if output_prob:
