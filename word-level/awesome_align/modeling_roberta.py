...
from awesome_align.modeling import BertContrastiveHead, BertEmbeddings, BertLayerNorm, BertModel, BertPreTrainedModel, gelu, BertPSIHead, BertGuideHead, PAD_ID, CLS_ID, SEP_ID
...
class RobertaForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.contrastive_layer = BertContrastiveHead(config)
    ...
    def forward(
        ...
        wc_loss = self.contrastive_layer(outputs_src, outputs_tgt, inputs_src, inputs_tgt, guide=guide, extraction=extraction, softmax_threshold=softmax_threshold, softmax_omit_threshold=softmax_omit_threshold, temp=temp)
        return wc_loss

    def get_aligned_word(self, inputs_src, inputs_tgt, bpe2word_map_src, bpe2word_map_tgt, device, src_len, tgt_len, align_layer=8, extraction='softmax', softmax_threshold=0.001, softmax_omit_threshold=0, test=False, output_prob=False, word_aligns=None):
        batch_size = inputs_src.size(0)
        ...
        if word_aligns is None:
            ...
            with torch.no_grad():
                ...
                classification_probs = attention_probs_inter.clone()
                attention_probs_inter = attention_probs_inter == 1.0
            ...
            word_aligns = []
            attention_probs_inter = attention_probs_inter[:, 0, 1:-1, 1:-1] # remove the prob between <s> </s> and tokens
