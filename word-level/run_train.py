        ...
        if args.cache_data and os.path.isfile(cache_fn) and not args.overwrite_cache:
            ...
            for line_id, line in tqdm(enumerate(lines), desc='Loading data', total=len(lines)):
                if len(line) > 0 and not line.isspace() and len(line.split(' ||| ')) == 2:
                    ...
                    ids_src, ids_tgt = tokenizer.prepare_for_model(list(itertools.chain(*wid_src)), return_tensors='pt', max_length=128, truncation=True)['input_ids'], tokenizer.prepare_for_model(list(itertools.chain(*wid_tgt)), return_tensors='pt', max_length=128, truncation=True)['input_ids']
                    # tokenizer.max_len
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples):
        ...
        if args.n_gpu > 1 or args.local_rank != -1:
            guides = model.module.get_aligned_word(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device, src_len, tgt_len, align_layer=args.align_layer, extraction=args.extraction, softmax_threshold=args.softmax_threshold, softmax_omit_threshold=args.softmax_omit_threshold, word_aligns=word_aligns)
        else:
            guides = model.get_aligned_word(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device, src_len, tgt_len, align_layer=args.align_layer, extraction=args.extraction, softmax_threshold=args.softmax_threshold, softmax_omit_threshold=args.softmax_omit_threshold, word_aligns=word_aligns)
    ...
    for _ in range(int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if args.train_wc:
                ...
                loss = model(inputs_src=inputs_src, inputs_tgt=inputs_tgt, attention_mask_src=attention_mask_src, attention_mask_tgt=attention_mask_tgt, guide=guide, align_layer=args.align_layer, extraction=args.extraction, softmax_threshold=args.softmax_threshold, softmax_omit_threshold=args.softmax_omit_threshold, train_so=args.train_so, train_co=args.train_co, temp=args.temperature)
...
def main():
    ...
    parser.add_argument("--train_wc", action="store_true")
    # Supervised settings
    parser.add_argument(
    ...
    parser.add_argument(
        "--softmax_omit_threshold", type=float, default=0
    )
    parser.add_argument(
        "--temperature", default=1, type=float, help="Temperature in contrastive learning."
    )
    parser.add_argument(
    ...
    if args.do_train:
        ...
        if args.local_rank == -1 or torch.distributed.get_rank() == 0:
            logger.info("Original saving path is: %s" % (args.output_dir))
            # if os.path.exists(args.output_dir) == True:
            #     for fix in range(1000):
            #         if os.path.exists(args.output_dir + str(fix)) == False:
            #             args.output_dir = args.output_dir + str(fix)
